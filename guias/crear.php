...
$transportistas = $conn->query("SELECT * FROM transportistas");
...
if(isset($_POST['guardar'])){
  ...
  $q = $conn->query("SELECT IFNULL(MAX(correlativo),0)+1 AS next 
                     FROM guias_remision WHERE tipo='$tipo'");
  $row = $q->fetch_assoc();
...
<div class="card shadow-lg border-0 erp-card">
<div class="card-header erp-header">
<h4><i class="fas fa-file-invoice"></i> Nueva Guía de Remisión</h4>
</div>
...
<select name="transportista_id" id="transportista" class="form-control" onchange="cargarTransportista()" required>
...
<button name="guardar" class="btn btn-primary px-4">
