...
// =======================
// CORRELATIVO ND BOLETA
// =======================
$q = $conn->query("SELECT IFNULL(MAX(correlativo),0)+1 AS next FROM notas_debito_boleta");
$row = $q->fetch_assoc();
...
$data = [
  ...
  "tipo_de_nota_de_debito" => "02", // aumento en el valor
  ...
  "items" => [
    [
      "unidad_de_medida" => "ZZ",
      "codigo" => "001",
      "descripcion" => "AUMENTO DE VALOR BOLETA ".$b['serie']."-".$b['correlativo'],
      "cantidad" => 1,
...
$conn->query("
INSERT INTO notas_debito_boleta
(boleta_id, serie, correlativo, respuesta_sunat, estado_sunat)
VALUES
($id,'BBB1',$correlativo_nd,'".$conn->real_escape_string($response)."','$estado')
");
...
if($estado=="ACEPTADO"){
  ...
  echo $r['sunat_description'] ?? "Nota de débito boleta enviada con observaciones";
}
